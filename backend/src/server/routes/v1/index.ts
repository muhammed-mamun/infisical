import {
  ...
  registerAwsConnectionRouter,
  registerGitHubConnectionRouter
} from "src/server/routes/v1/app-connection-routers";
...
export const registerV1Routes = async (server: FastifyZodProvider) => {
  ...
  await server.register(
    async (appConnectionsRouter) => {
      ...
      await appConnectionsRouter.register(registerAwsConnectionRouter, { prefix: "/aws" });
      await appConnectionsRouter.register(registerGitHubConnectionRouter, { prefix: "/github" });
    },
